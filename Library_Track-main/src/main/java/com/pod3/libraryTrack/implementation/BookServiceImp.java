...
public class BookServiceImp implements BookService {
	...
	public Book saveBook(Book book, Long categoryId) {
		...
		// Initialize available copies to total copies
		book.setAvailableCopies(book.getTotalCopies());
		// Set initial availability status
		book.setAvailabilityStatus(book.getAvailableCopies() > 0 ? AvailabilityStatus.Available : AvailabilityStatus.NotAvailable);
	...
	public Book updateBook(Long categoryId, Long bookId, Book updatedBook) {
		...
		// Update total copies if provided and different
		if (updatedBook.getTotalCopies() > 0 && updatedBook.getTotalCopies() != book.getTotalCopies()) {
			int oldTotal = book.getTotalCopies();
			int newTotal = updatedBook.getTotalCopies();
			...
			// Auto-update availability based on available copies
