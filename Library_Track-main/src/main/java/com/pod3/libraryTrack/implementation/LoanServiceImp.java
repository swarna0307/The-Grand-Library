...
public class LoanServiceImp implements LoanService {
	...
	public LoanDto createLoan(Loan loan) {
		...
		// If no copies remain, mark as Loaned
		if (newCopies == 0) {
		...
		if (loan.getStatus() != null) {
			...
			if (loan.getStatus() == LoanStatus.Returned) {
				...
				if (book != null) {
					int newCopies = book.getCopies() + 1;
					book.setCopies(newCopies);
					// If book was Loaned or NotAvailable, restore to Available
					if (book.getAvailabilityStatus() == AvailabilityStatus.Loaned || book.getAvailabilityStatus() == AvailabilityStatus.NotAvailable) {
